...
# Inputs
otu <- read.delim("Fungi_otutab_rare.txt", row.names = 1, check.names = FALSE)
metadata <- read.delim("Fungi_metadata_raw.txt", header = TRUE,  stringsAsFactors = FALSE)
...
# Save
ggsave("Fungi_PCoA_PERMANOVA.pdf", plot = p1,
...
write.csv(df[, c("SampleID", "PCoA1", "PCoA2", "Group", "StudyID")],
          ...
          row.names = FALSE)
cat("PCoA coordinates saved to 'Fungi_PCoA_Coordinates.csv'\n")
...
# Output
write.csv(dispersion_result, "Fungi_beta_dispersion_result.csv", row.names = FALSE)
...
# output
ggsave(
  filename = "Fungi_beta_dispersion_custom_boxplot.pdf",
  plot = p,
  width = 6,
  height = 5
)
...
write.table(alpha_meta,
            "Fungi_alpha_diversity.txt", sep = "\t", quote = FALSE, row.names = FALSE)

##  P value + BH
vars <- c("Observed", "Shannon", "PD")
...
write.table(stat_res, "Fungi_alpha_Wilcoxon_BH.txt", sep = "\t", quote = FALSE, row.names = FALSE)
...
write.xlsx(summary_df, "Fungi_Schoener_Index_Summary.xlsx", rowNames = FALSE)
write.xlsx(schoener_df, "Fungi_Schoener_Index_All_Pairs.xlsx", rowNames = FALSE)
write.xlsx(group_within, "Fungi_Schoener_Index_Within_Group_Pairs.xlsx", rowNames = FALSE)
...
# Output
ggsave("Fungi_Schoener_within_groups_boxplot.pdf", p, width = 6, height = 5)
...
asv_table <- read.delim("Fungi_otutab_rare.txt",
                        row.names = 1,
                        check.names = FALSE,
                        stringsAsFactors = FALSE)

taxonomy <- read.delim("Fungi_taxonomy.txt",
                       row.names = 1,
                       check.names = FALSE,
                       stringsAsFactors = FALSE)
...
run_aldex_on_level <- function(taxa_level) {
  ...
  res$Significance <- ifelse(res$we.ep < 0.001, "***",
                             ifelse(res$we.ep < 0.01, "**",
                                    ifelse(res$we.ep < 0.05, "*", "")))
  write.csv(res,
            paste0("Fungi_", taxa_level, "_aldex_result.csv"),
            row.names = FALSE)
...
plot_top10_effect <- function(res, taxa_df, level_name) {
  ...
  ## Remove Unassigned / Unclassified
  ...
  ggsave(paste0("Fungi_", level_name, "_Top10_effect_noUnassigned.pdf"),
         p,
         width = 7,
         height = 5,
         device = cairo_pdf)
...
for (g in names(otu_split)) {
  fn <- paste0("Fungi_otu_group_", g, ".txt")
...
build_network <- function(
    ...
    q_thr = 0.01,
    allow_negative = TRUE,
    output_dir = "Fungi_CK_network_out",
    prefix = "Fungi_CK"
){
...
otu_file <- "Fungi_otu_group_CK.txt"
tax_file <- "Fungi_taxonomy.txt"
...
res <- build_network(
  ...
  output_dir = "Fungi_CK_network_out", prefix = "Fungi_CK"
)
...
build_network <- function(
    ...
    q_thr = 0.01,
    allow_negative = TRUE,
    output_dir = "Fungi_TR_network_out",
    prefix = "Fungi_TR"
){
...
otu_file <- "Fungi_otu_group_TR.txt"
tax_file <- "Fungi_taxonomy.txt"
...
res <- build_network(
  ...
  prevalence_prop = 0, total_abund_prop = 0,  
  rho_thr = 0.6, q_thr = 0.01, allow_negative = TRUE,
  output_dir = "Fungi_TR_network_out", prefix = "Fungi_TR"
)
...
head(res$nodes)   
#   nodes_num edges_num positive.corr_num positive.corr_prop negative.corr_num negative.corr_prop average_degree
# 1       325     19720             19720                  1                 0                  0       121.3538
#   average_shortest_path_length network_diameter network_density clustering_coefficient modularity
# 1                    0.9902004         3.293417       0.3745489              0.9572833  0.4012044
...
build_network <- function(
    ...
    q_thr = 0.01,
    allow_negative = TRUE,
    output_dir = "Fungi_Subnetwork_out",
    prefix = "Fungi_Subnetwork"
){
...
otu_file <- "Fungi_otutab_rare.txt"    
tax_file <- "Fungi_taxonomy.txt"       

first <- readLines(otu_file, n = 1)
is_header <- grepl("^#?OTU", first, ignore.case = TRUE) || grepl("Fungi_ASV", first)
...
write.csv(metrics_df, "Fungi_sample_subnetwork_metrics.csv", row.names=FALSE)
...
## Read and merge data
metrics_df <- read.csv("Fungi_sample_subnetwork_metrics.csv", header = TRUE)
...
p_single
p_single_modified <- p_single + coord_cartesian(ylim = c(0, 800))
print(p_single_modified)
ggsave("Fungi_Subnetwork_nodes_num.pdf", p_single_modified, width = 6, height = 5)

###### edges_num
p_single2 <- plot_metrics_df(
  data = net_meta,
  yvar = "edges_num",   
  stat_table = stat_res,
  ylab = "edges_num"        
...
p_single2
p_single_modified2 <- p_single2 + coord_cartesian(ylim = c(0, 78000))
print(p_single_modified2)
ggsave("Fungi_Subnetwork_edges_num.pdf", p_single_modified2, width = 6, height = 5)


################################################################################################### Mean and SD Calculation
library(dplyr)

df <- read.csv("Fungi_sample_subnetwork_metrics.csv", stringsAsFactors = FALSE)
